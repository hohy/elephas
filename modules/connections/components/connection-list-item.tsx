import { Button, ListItem, SizableText, Text, View, XStack, YGroup } from "tamagui"
  ...
  return (
    <View width={60} backgroundColor={"$red10Dark"}>
      <Animated.View
        ...
        <Button borderRadius={22} width={44} height={44} padding={0} backgroundColor={"$red10Light"} onPress={handlePress}>
          <XCircle color={"$color"} />
        </Button>
      </Animated.View>
    </View>
...
export default function ConnectionsListItem(props: {connection: Connection, index: number}) {
  ...
  return <YGroup.Item key={props.connection.id}>
  <ListItem enterStyle={{ scale: 1.5, y: 50 * props.index, opacity: 0 }} animation="medium" onLongPress={showConnectionActions} padding={0}>
    <Swipeable renderRightActions={renderRightActions(() => deleteConnection(props.connection.id))} friction={2} containerStyle={{ flex: 1, height: '100%' }}> 
     <XStack justifyContent="space-between" alignItems="center" width={"100%"} paddingHorizontal={18} paddingVertical={10} backgroundColor={"$background"} >
        ...
        { props.connection.environment && <XStack borderRadius={4} overflow="hidden"><Text padding={"$1.5"} overflow={"hidden"}>{props.connection.environment}</Text></XStack>}
        <ChevronRight/>
      </XStack>
    </Swipeable>
  </ListItem>
